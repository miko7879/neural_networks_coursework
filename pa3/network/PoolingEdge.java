...
public class PoolingEdge extends Edge {
    ...
    public void propagateBackward(double[][][][] delta) {
        for(int image = 0; image < delta.length; image++){
            for(int channel = 0; channel < delta[image].length; channel++){
                for(int column = 0; column < delta[image][channel].length; column++){
                    for(int row = 0; row < delta[image][channel][column].length; row++){
                        passDelta(image, channel, column, row, delta);
                    }
                }
            }
        }
    ...
    private void passDelta(int image, int channel, int column, int row, double[][][][] delta){
        for(int poolColumn = 0; poolColumn < poolSize; poolColumn++){
            for(int poolRow = 0; poolRow < poolSize; poolRow++){
                inputNode.delta[image][channel][column * stride + poolColumn][row * stride + poolRow] = delta[image][channel][column][row]*poolDelta[image][channel][column*stride + poolColumn][row*stride + poolRow];
            }
        }
    }
