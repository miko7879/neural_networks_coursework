...
public class ConvolutionalEdge extends Edge {
    ...
    public void initializeKaiming(double range) {
        ...
        for(int channel = 0; channel < weight.length; channel++){
            for(int column = 0; column < weight[channel].length; column++){
                for(int row = 0; row < weight[channel][column].length; row++){
                    weight[channel][column][row] = range*weightGen.nextGaussian();
                }
            }
        }
    ...
    public void initializeXavier(double range) {
        ...
        for(int channel = 0; channel < weight.length; channel++){
            for(int column = 0; column < weight[channel].length; column++){
                for(int row = 0; row < weight[channel][column].length; row++){
                    weight[channel][column][row] = range*(2.0*weightGen.nextDouble() - 1);
                }
            }
        }
    ...
    public void propagateForward(double[][][][] inputValues) {
        ...
        for(int image = 0; image < outputNode.inputValues.length; image++){
            for(int channel = 0; channel < outputNode.inputValues[image].length; channel++){
                for(int column = 0; column < outputNode.inputValues[image][channel].length - 2 * pad; column++){
                    for(int row = 0; row < outputNode.inputValues[image][channel][column].length - 2 * pad; row++){
                        for(int filterChannel = 0; filterChannel < weight.length; filterChannel++){
                            for(int filterColumn = 0; filterColumn < weight[filterChannel].length; filterColumn++){
                                for(int filterRow = 0; filterRow < weight[filterChannel][filterColumn].length; filterRow++){
                                    outputNode.inputValues[image][channel][column + pad][row + pad] += weight[filterChannel][filterColumn][filterRow]*inputValues[image][channel + filterChannel][column + filterColumn][row + filterRow];
                                }
                            }
                        }
                    }
    ...
    public void propagateBackward(double[][][][] delta) {
        ...
        int pad = outputNode.padding;
        for(int image = 0; image < delta.length; image++){
            for(int channel = 0; channel < delta[image].length; channel++){
                for(int column = 0; column < delta[image][channel].length - 2 * pad; column++){
                    for(int row = 0; row < delta[image][channel][column].length - 2 * pad; row++){
                        double delt = delta[image][channel][column + pad][row + pad];
                        for(int filterChannel = 0; filterChannel < weightDelta.length; filterChannel++){
                            for(int filterColumn = 0; filterColumn < weightDelta[filterChannel].length; filterColumn++){
                                for(int filterRow = 0; filterRow < weightDelta[filterChannel][filterColumn].length; filterRow++){
                                    weightDelta[filterChannel][filterColumn][filterRow] += delt * inputNode.outputValues[image][channel + filterChannel][column + filterColumn][row + filterRow];
                                    inputNode.delta[image][channel][column + filterColumn][row + filterRow] += delt * weight[filterChannel][filterColumn][filterRow];
                                }
                            }
                        }
                    }
