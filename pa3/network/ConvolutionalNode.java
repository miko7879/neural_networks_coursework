                ...
                if (training) {
                    ...
                    for (int i = 0; i < batchSize; i++) {
                        for (int z = 0; z < sizeZ; z++) {
                            for (int y = 0; y < sizeY; y++) {
                                for (int x = 0; x < sizeX; x++) {
                                    var += (inputValues[i][z][y][x] - mu) * (inputValues[i][z][y][x] - mu);
                                }
                            }
                        }
                    }
                    ...
                    mu_b = mu;
                    sigma2_b = var;
                    //Find xHat and afterBatchNorm
                    for (int i = 0; i < batchSize; i++) {
                        for (int z = 0; z < sizeZ; z++) {
                            for (int y = 0; y < sizeY; y++) {
                                for (int x = 0; x < sizeX; x++) {
                                    xHat[i][z][y][x] = (inputValues[i][z][y][x] - mu) / Math.sqrt(var + epsilon);
                    ...
                    mu_running = alpha*mu + (1 - alpha)*mu_running;
                    sigma2_running = alpha*var + (1 - alpha)*sigma2_running;
                } else {
                    ///Find xHat and afterBatchNorm
                    for (int i = 0; i < batchSize; i++) {
                        for (int z = 0; z < sizeZ; z++) {
                    ...
                    for (int z = 0; z < sizeZ; z++) {
                        for (int y = 0; y < sizeY; y++) {
                            for (int x = 0; x < sizeX; x++) {
                                ...
                                if(val > 0 && val < 5) { derivative[i][z][y][x] = 1; }
                            }
                        }
                    }
                }
                ...
                if (training) {
                    for (int i = 0; i < batchSize; i++) {
                        for (int z = 0; z < sizeZ; z++) {
                            for (int y = 0; y < sizeY; y++) {
                                for (int x = 0; x < sizeX; x++) {
                                    double testVal = generator.nextDouble();
                                    if(testVal < (1.0 - dropoutRate)){
    ...
    public void propagateBackward() {
        ...
        if (nodeType == NodeType.HIDDEN) {
            ...
            if (useBatchNormalization) {
                ...
                //Store the deltaGammaSum
                double deltaGammaSum = 0; 
                
                //Store the deltaMuSum
                double deltaMuSum = 0;
                
                //Store the deltaMu for each cell
                double[][][][] deltaMu = new double[batchSize][sizeZ][sizeY][sizeX];
                
                //Calculate the number of cells
                int numCells = batchSize*sizeZ*sizeY*sizeX;
                
                //First pass to calculate betaDelta, gammaDelta, and sum_delta_gamma
                for (int i = 0; i < batchSize; i++) {
                    for (int z = 0; z < sizeZ; z++) {
                        for (int y = 0; y < sizeY; y++) {
                            for (int x = 0; x < sizeX; x++) {
                                ...
                                deltaGammaSum += delta[i][z][y][x]*gamma*(inputValues[i][z][y][x] - mu_b);
                            }
                        }
                    }
                }
                
                //Some constants for second pass - through
                double denom = Math.sqrt(sigma2_b + epsilon);
                double deltaGammaSumNorm = deltaGammaSum/(numCells*denom*denom*denom);
                                
                //Second pass to calculate the deltaMuSum and deltaMu for each cell
                for (int i = 0; i < batchSize; i++) {
                    for (int z = 0; z < sizeZ; z++) {
                        for (int y = 0; y < sizeY; y++) {
                            for (int x = 0; x < sizeX; x++) {
                                double deltaMu_k = delta[i][z][y][x]*gamma/denom - (inputValues[i][z][y][x] - mu_b)*deltaGammaSumNorm;
                                deltaMu[i][z][y][x] = deltaMu_k;
                                deltaMuSum += deltaMu_k;
                            }
                        }
                    }
                }
                ...
                //Third pass to calculate the actual delta for each cell
                for (int i = 0; i < batchSize; i++) {
                    for (int z = 0; z < sizeZ; z++) {
                        for (int y = 0; y < sizeY; y++) {
                            for (int x = 0; x < sizeX; x++) {
                                delta[i][z][y][x] = deltaMu[i][z][y][x] - deltaMuSum/numCells;
